...
int main(){
    ...
    if( spi_host_init(spi_flash, 1)!= SPI_FLAG_SUCCESS) return EXIT_FAILURE;
    ...
    spi_slave_write(spi_flash, buffer_write_to, buffer_write_from, DATA_LENGTH_B);
    ...
    for( i=0; i<bpb_length_b; i++){
        spi_slave_request_read(spi_flash,&buffer_read_from[i],  1, DUMMY_CYCLES );
        spi_wait_for_rx_watermark(spi_flash);
        buffer_read_to[i] = spi_copy_byte(spi_flash, i%4 );
    }
    ...
    for( i=0; i<wpw_length_w; i++){
        spi_slave_request_read(spi_flash,&((uint32_t*)buffer_read_from)[start_w + i],  4, DUMMY_CYCLES );
        spi_wait_for_rx_watermark(spi_flash);
        ((uint32_t*)buffer_read_to)[ start_w + i ] = spi_copy_word(spi_flash);
    }
    // Check if the read values are ok.
    ...
    for( i=0; i<chunks_n; i++){
        spi_slave_request_read(spi_flash,&((uint32_t*)buffer_read_from)[start_w + i*chunk_w],  chunk_w*4, DUMMY_CYCLES );
        spi_wait_for_rx_watermark(spi_flash);
        spi_copy_words( spi_flash, &((uint32_t*)buffer_read_to)[start_w + i*chunk_w],  chunk_w );
    }
    // Check if the read values are ok.
