...
class I2cBusScanner:
    ...
    def scan(cls, url: str, smb_mode: bool = True) -> None:
        ...
        try:
            ...
            # Switching cheep mux to I2C mode
            
            cb.enable_chewing_gums()

            for ID in cheep.I2C_TREE:
                slaves = []
                print("Scanning {} ({})".format(ID, cheep.I2C_TREE[ID]["label"]))
                cb.set_i2c_mux(ID)
                ...
                while True:
                    ...
                    row += columns
                cg = cb.get_chewing_gum_by_name(ID)
                if cg != None:
                    print("{}".format(cg))                    
                    #cg_id = eeprom.get_id()
                    #print("EEPROM EUI64: {}".format(eui64))
                    #print("Chewing Gum ID: 0x{:4x}".format(cg_id))
                print("\n")           
        finally:
            i2c.terminate()
