...
ln1 = ax1.plot(x, y_freq, 'k-', label='Frequency (kHz)')
ax1.set_xlabel('Input Voltage (mV)')
ax1.set_ylabel('Oscillation Frequency (kHz)')

# Plot Power on Right Y-axis
ax2 = ax1.twinx()
ln2 = ax2.plot(x, y_vco, color='gray', ls='--', label='P_vco (µW)')
ln3 = ax2.plot(x, y_cnt, color='0.6', ls=':', label='P_counter (µW)')
ln4 = ax2.plot(x, y_tot, color='0.4', ls='-.', label='P_total (µW)')
...
plt.tight_layout()
plt.savefig('figs/vco_transfer.png')